...
## GSC – indeksowanie (analiza zrzutów z 2026-09-04, dane GSC z 28.08.2026)
- Stan: 29 zindeksowanych, 15 nie: „strona zawiera przekierowanie" 10, „alternatywna strona z prawidłowym canonical" 3, „zeskanowana, jeszcze nie zindeksowana" 2.
- Przyczyna 10 przekierowań (SPRAWDZONE curl): serwer (LiteSpeed) robi 301 z `/wukotargowek` na `/wukotargowek/`, canonical wskazuje wersję z ukośnikiem, a linki wewnętrzne (Navigation, Footer, Locations, DistrictContent „inne dzielnice", linki w artykułach) prowadziły na wersję BEZ ukośnika. Google zbierał więc adresy bez ukośnika i raportował je jako przekierowania. POPRAWIONE 2026-09-04: wszystkie linki wewnętrzne z ukośnikiem, `trailingSlash: 'always'` w astro.config.mjs, breadcrumb schema na podstronach dzielnic z ukośnikiem. Konwencja od teraz: KAŻDY link wewnętrzny kończy się `/`.
- Przyczyna 3 „alternatywnych" (PRZYPUSZCZENIE, potwierdzone dwa mechanizmy na serwerze): (a) `www.wukowarszawa.pl` zwracało 200 zamiast 301 na wersję bez www; (b) stary `.htaccess` z czasów React SPA miał fallback „nieistniejący adres → index.html z kodem 200" — każdy zmyślony URL zwracał stronę główną (soft 404). POPRAWIONE 2026-09-04: nowy `public/.htaccess` — 301 www→bez www i http→https, usunięty fallback SPA, `ErrorDocument 404 /404.html`, stare adresy `.html` przekierowane od razu na wersję z ukośnikiem (wcześniej podwójny skok), `reviews.html` → `/opinie/`. Nowa strona `src/pages/404.astro`.
- 2 „zeskanowane, jeszcze nie zindeksowane" — najpewniej nowe artykuły poradnika (nie widać adresów na zrzucie); poczekać, ewentualnie „Poproś o zindeksowanie" w GSC.
- PO WDROŻENIU sprawdzić curl: `https://www.wukowarszawa.pl/` → 301, `https://wukowarszawa.pl/asdf` → 404, `https://wukowarszawa.pl/wukotargowek.html` → 301 na `/wukotargowek/`. Potem w GSC kliknąć „Sprawdź poprawkę" przy obu przyczynach.
